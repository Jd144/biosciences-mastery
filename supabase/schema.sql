...
-- ============================================================
-- 12. AI USAGE LOG (daily rate limiting: free = 5/day, premium = unlimited)
-- ============================================================
...
-- AI usage log: users see and insert only their own
CREATE POLICY "Users can read own ai_usage_log" ON public.ai_usage_log FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Users can insert own ai_usage_log" ON public.ai_usage_log FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
